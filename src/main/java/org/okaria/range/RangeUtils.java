...
public interface RangeUtils {
	...
	default long[][] checkRanges(long[][] ranges) {
		long[][] newRange = new long[ranges.length][2];
		int items = 0;
		for (long[] ls : ranges) {
			if (ls[0] - ls[1] >= 0)
				continue;
		...
		for (int i = 0; items < newRange.length; i++, items++) {
			long[][] splitRanges = SubRange.subrange(newRange[i][0], newRange[i][1], 2);
			newRange[i] = splitRanges[0];
			newRange[items] = splitRanges[1];
		}
